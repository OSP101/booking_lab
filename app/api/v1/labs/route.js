...
const postLabHandler = async (req) => {
    try {
        ...
        const promisePool = mysqlPool.promise();
        // ตรวจสอบค่าที่จำเป็น
        const { createdAt, name, image, subject, status, room, redirect, type } = body;
        ...
        const [labResult] = await promisePool.query(
            'INSERT INTO Labs (id, createdAt, name, image, subject, status, room, redirect, type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
            [id, createdAt, name, image, subject, status, room, redirect, type]
        );
